...
## Setup Performance + P&L Heatmap + Coach TTS + Discord !gex (2026-06 fork) — DONE:
- SETUP PERFORMANCE (Pin Setups): GET /api/insights/setups aggregates executed trades by detected_setup/strategy_name -> {setup, trades, pnl, win_rate, avg_pnl}, sorted by pnl desc. Screen app/setups.tsx (Top Setup card + ranked list), linked from dashboard "Setup Performance". Fixed theme bug (fs.xxl -> fs["2xl"]). Verified via curl + screenshot.
- P&L HEATMAP: GET /api/insights/heatmap -> day-of-week aggregation (Mon–Sun, weekend only if used) + session buckets (Open <10, Mid-AM 10–12, Midday 12–14, PM 14–16, Late 16+) using _parse_hour(trade_time); returns day_stats, session_stats, grid[session][day] {pnl,trades,win_rate}, best_day/worst_day/best_session, has_time. Screen app/heatmap.tsx: best/toughest-day callouts, day-of-week bar chart, session×day heat grid (green=gains, red=losses, intensity by |pnl|). Linked from dashboard "P&L Heatmap". Verified via curl + screenshot (renders with real data).
- COACH VOICE REPLY + SPOKEN RECAP (TTS): POST /api/coach/speak {text, voice?} -> {audio_base64 (mp3), mime}. Premium-gated, AI rate-limited, strips markdown/FOLLOWUPS, caps 4000 chars. Uses emergentintegrations OpenAITextToSpeech (model tts-1, voice onyx, Emergent LLM key). Frontend hook src/hooks/useSpeak.ts (fetches audio, plays via expo-audio createAudioPlayer; data-URI on web, temp .mp3 file on native; per-id loading/playing state). Speaker icon on each AI coach bubble (coach.tsx) + "Listen" button on Session Report (report.tsx) reads the day back. Verified: /api/coach/speak 200 OK from app, valid ~88KB mp3, no console errors.
- DISCORD !gex COMMAND: discord_bot.build_client now takes gex_summary callable; !gex [SYM] handler posts an embed with regime (Positive/Negative gamma + plain-English implication), spot, gamma flip, call/put walls for SPY/SPX/XSP. Backend helper _gex_summary(sym) reads gex_snapshots. NOTE: bot requires "Message Content Intent" enabled in Discord Developer Portal (currently raising PrivilegedIntentsRequired at startup — user config item, does NOT affect the FastAPI API).

## STILL QUEUED (this fork):
- Auto-tag emotion from voice-note TONE -> P&L correlation: NOT built. Whisper only returns transcript text (no true audio-tone/prosody), and voice notes aren't currently stored/linked to a trade or day. Needs a design decision (store voice journals + link to day/trade; infer emotion from transcript sentiment vs. real prosody). Native voice recording also can't be fully tested in Expo Go/web.
- Discord streak/milestone shout-outs: user said they're NOT worried about streak nudges — deferred.
- Blocked (platform, not code): user's Android APK pulled a stale build — escalated to Emergent Support. Do NOT ask user to test native-only features (Share Intent, Voice recording) until they confirm support fixed the build pipeline.

## Free Premium Trial + Annual Upsell Banner + Heatmap Drill-Down (2026-06 fork) — DONE:
- FREE PREMIUM TRIAL (self-serve, no card): POST /api/user/start-trial — one-time (trial_used guard), free tier only; sets trial_premium_until = now+TRIAL_DAYS(7). effective_tier() now grants premium while trial_premium_until is in the future (auto-downgrades on expiry). public_user exposes trial_premium_until + trial_used. Dashboard shows a "Try Premium free for 7 days" CTA for free users who haven't trialed; tapping calls start-trial, refreshes auth, and unlocks Premium (verified in UI: Market Sentiment appears after start). Re-use is blocked (400).
- ANNUAL UPSELL BANNER: GET /api/upsell/annual — show:true only when raw tier is pro/premium AND the earliest COMPLETED monthly payment (tier 'pro'/'premium' in db.payments) is >= UPSELL_MIN_MONTHS(2) old; returns months_active, monthly/annual amounts, annual_monthly_equiv, savings_amount, savings_pct (from STRIPE_PACKAGES). Dashboard banner ("SAVE X% · Go annual & save $Y/yr") routes to /(tabs)/profile?billing=annual (profile now reads the billing param to preselect Annual). Dismiss X snoozes 14 days via AsyncStorage key bca_annual_upsell_snooze. Verified with a seeded 90-day-old completed payment (savings math correct: $22.91/mo vs $24.99/mo, save 8%). Seed data removed after verification.
- HEATMAP DRILL-DOWN: GET /api/insights/heatmap/trades?day=&session= returns the executed trades in that weekday×session bucket (id, symbol, pnl, trade_time, setup, grade) + total_pnl/count. heatmap.tsx cells are now Pressable (disabled when empty) and open a bottom-sheet Modal listing the trades (grade badge color-coded via GRADE_COLORS, tap a row -> /debrief/{id}). Verified in UI (Sun · Mid-AM -> $933 · 4 trades).
- NOTE: Voice Emotion Insight (emotion from voice-note tone -> P&L) still NOT built — needs a design decision + depends on native voice recording (untestable until build pipeline fixed).

## Voice Emotion Journal + Trial Countdown + Heatmap Filters (2026-06 fork) — DONE:
- VOICE EMOTION JOURNAL (Premium): POST /api/journal/voice-note {text? | audio_base64,ext?} — transcribes audio via Whisper if given, then an LLM (Claude) infers emotion from the WORDS + TONE + overall feel, constrained to the existing EMOTIONS set [Calm,Confident,Disciplined,FOMO,Anxious,Greedy,Revenge,Bored], returning emotion + tone descriptor + one-line reason; stored in db.voice_notes {text,emotion,tone,reason,date,created_at}. GET /api/journal/voice-notes and GET /api/insights/emotion-voice correlate each note to that DAY's realized P&L (sum of executed, non-pending trades on the note's date) → per-emotion count/avg_pnl/total_pnl/win_rate(green days) + best/worst emotion + recent notes. DELETE /api/journal/voice-note/{id}. Screen app/voice-journal.tsx: mic (records→transcribes→fills editable box; native-only) OR type a note, "Save & read my mood" → shows Best/Watch-out mood callouts, Mood vs P&L list, and recent notes (emotion chip, tone, day P&L, reason, delete). Linked from dashboard "Voice Journal & Mood". Verified via UI (typed note → "Disciplined", tone "calm and measured") + curl (Anxious note).
- TRIAL COUNTDOWN: dashboard strip "X days of Premium left · Keep Premium" shown when raw_tier is free AND trial_premium_until is in the future (days derived client-side from public_user.trial_premium_until); routes to profile plans. Verified in UI ("7 days of Premium left").
- HEATMAP FILTERS: GET /api/insights/heatmap now accepts ?symbol= and ?strategy_id= and also returns available `symbols` + `strategies` options (computed pre-filter); /insights/heatmap/trades accepts the same filters. heatmap.tsx shows a horizontal chip bar (All + symbols + strategy names); tapping reloads the grid/bars/callouts for that filter (useEffect on filter state, since useAutoRefresh holds load in a ref). Verified in UI (SPY filter → Best Sun $534/100%WR, Toughest Mon -$197).

## Coach Mood Nudge + Weekly Mood Recap + Heatmap Grade Filter + Keep-Premium Offer (2026-06 fork) — DONE:
- COACH MOOD NUDGE: GET /api/coach/mood-nudge (Premium) — takes the trader's most recent voice-note emotion; if that emotion has >=2 notes and a NEGATIVE avg day P&L, returns {show, emotion, count, avg_pnl, message}. coach.tsx fetches it in load() and shows a dismissible amber banner at the top of the chat ("You logged feeling anxious recently. On your anxious days you've averaged -$131..."). Verified in UI.
- WEEKLY MOOD RECAP: _compute_weekly_summary now also returns dominant mood (most common emotion among voice_notes in the last 7 days) + that mood's avg day P&L (mood, mood_pnl, mood_count); _digest_html adds a "Dominant mood" row (green/red avg/day). Flows into the weekly digest email + test-digest. (Email only sends when there ARE trades in the last 7 days.)
- HEATMAP GRADE FILTER: /insights/heatmap and /insights/heatmap/trades now also accept ?grade= and heatmap returns a `grades` option list. heatmap.tsx adds a SECOND, independent chip row ("Grade: All A C D F", colored via GRADE_COLORS) that combines with the symbol/strategy filter. Verified in UI (grade A -> only A trades: Fri -$265).
- KEEP-PREMIUM OFFER: new src/utils/checkout.ts startCheckout(tier, offer?) reusable Stripe flow. create-checkout-session now takes offer:"trial" — for a user still inside their trial_premium_until it applies the first-month promo discount EVEN IF the launch promo window has passed, and does NOT grant another free trial (sub_data={}). Dashboard: when trialCountdown <= 2 days, the strip becomes a "Trial ends in X · Keep Premium — 20% off 1st month" card that calls startCheckout("premium","trial") directly (one-tap); >2 days keeps the simple countdown routing to profile. Verified in UI (offer card renders at 1 day left).

## Share-to-Notify (background capture + local notification) (2026-06 fork) — DONE (native-only, untested on web):
- GOAL: sharing a screenshot into the app should NOT force the app in front of the broker; instead analyze in the background and post a notification with the read, tappable to open.
- REALITY/CONSTRAINT: managed Expo + expo-share-intent still launches the app to receive the shared image (can't be fully invisible without a custom native share extension/activity, which our build pipeline doesn't support). Delivered best-effort: a fast minimal capture screen instead of dropping straight into the Coach.
- FLOW: ShareIntentHandler now routes shared images to app/capture.tsx (was /(tabs)/coach). capture.tsx: stashes the base64 to storage (PENDING_SHARE_KEY="bca_pending_share") + clears in-memory pending, requests notification permission (contextual), calls POST /trades/analyze-screenshot with preview:true (NEW flag → analyze WITHOUT saving; returns symbol/direction/pnl/setup_grade/detected_setup/ai_summary), fires a LOCAL notification (title "📸 Trade captured — SYM" + body "Setup · Grade X · ±$N\nTap to log it or ask your coach.", data.route="/(tabs)/coach"), then shows a "captured / Review now / Done" screen so the user can go back to their broker.
- NOTIFICATIONS: expo-notifications@0.32.17 (local only, no Firebase/push key). src/utils/notifications.ts (configure handler + Android "trades" channel, ensureNotifPermission per permissions contract, notifyLocal). src/components/NotificationRouter.tsx mounted in _layout handles taps (foreground + cold start via getLastNotificationResponseAsync) → router.push(data.route). app.json: added POST_NOTIFICATIONS (Android) + expo-notifications plugin (color #2E76E8).
- COACH pickup: coach.tsx added a bootstrap effect that reads PENDING_SHARE_KEY from storage on mount → setPending → existing "analyze + Did you take the trade?" flow runs (so tapping the notification, even from a cold start, lands on the full coach flow). Existing coach share pipeline unchanged otherwise.
- VERIFIED: backend preview flag returns the read and does NOT save (trade count unchanged 22→22); app boots + bundles clean on web (only a harmless expo-notifications web warning). NOT testable on web/Expo Go — requires a real device build (share intent + local notifications). User was informed.

## Deployment/EAS build fix (2026-06 fork):
- ROOT CAUSE of EAS iOS build failure: expo-share-intent was pinned at 8.0.1, which requires Expo SDK ^57, but this project is SDK 54 (the dev server printed a compatibility warning "v8.0.1 needs ^57, found 54.0.0"). A config-plugin/native version mismatch like this fails the EAS prebuild/build.
- FIX: downgraded expo-share-intent 8.0.1 -> 5.1.1 (SDK 54's supported line per the maintainer's compatibility table: SDK 54 -> 5.0+). Verified v5 API is unchanged for our usage (useShareIntent {resetOnBackground, disabled}, hasShareIntent, shareIntent.files, resetShareIntent) and the config-plugin still accepts our app.json options (androidIntentFilters, iosActivationRules). After a clean restart the SDK-mismatch warning is gone and the app bundles + boots cleanly.
- Also fixed a real latent bug surfaced by `tsc --noEmit`: app/voice-journal.tsx used api.delete() which doesn't exist (client method is api.del) — would have crashed voice-note deletion. Extended the frontend User type with raw_tier/trial_premium_until/trial_used/reward_pro_until (type-only; runtime already returned them).
- Deployment health check (deployment_agent): status warn, NO hard blockers. Non-blocking WARNs noted: backend loads a local .env (platform injects real env vars in prod - fine), and an N+1 leaderboard query (perf, not a blocker). backend binds 0.0.0.0:8001, /api prefix, DB name from env — all OK for Atlas.
- Remaining pre-existing type-only tsc warnings in src/utils/sound.ts (untouched, Metro strips types) — not a build blocker.

## EAS iOS IPA build failure fix #2 (2026-06 fork):
- REAL LOG this time: step #5 "eas-ipa-build" failed with "Failed to get Git root path with `git rev-parse --show-toplevel` ... code 128" (x2) then "build command failed / iOS IPA BUILD FAILED (exit 1)".
- git-128 is a BENIGN WARNING: it's emitted by @expo/fingerprint (node_modules/@expo/fingerprint/build/ProjectWorkflow.js) when the containerized EAS builder has no .git; fingerprint falls back and continues. NOT the fatal cause.
- LIKELY FATAL CAUSE: expo-share-intent's iOS config plugin injects a Share Extension native target WITH an App Group entitlement (com.apple.security.application-groups via withIosAppEntitlements / writeIosShareExtensionFiles). In Emergent's managed EAS iOS build, that app group + second-target signing isn't provisioned in the Apple account, so codesign/xcodebuild fails the IPA.
- FIX (code-level, no Docker): set expo-share-intent plugin option `disableIOS: true` in app.json (kept `androidIntentFilters: ["image/*"]`). Verified in v5.1.1 plugin index.js that disableIOS ONLY skips the 4 iOS extension mods (infoPlist activation, app-group entitlements, extension config, xcode target); the core ExpoShareIntent module stays autolinked so useShareIntent does NOT crash on iOS — it simply won't receive shares there. Android share-to-notify unaffected. App bundles clean.
- CONSEQUENCE: iOS users can't share a screenshot into the app from the iOS share sheet (best-effort iOS, per user choice). To enable it later, register an App Group in the user's Apple Developer account and re-enable the iOS extension. All other iOS features (Coach image picker, voice, local notifications, etc.) unaffected.
- NOTE: the true fatal line was not in the user's truncated log; this addresses the strongest suspect. If the iOS build still fails, request the full log around "build command failed" (the codesign/xcodebuild error).
